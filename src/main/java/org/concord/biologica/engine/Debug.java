...
public class Debug
{
	...
	**/
	...
	private static String [] prefixes = {"org.concord.biologica.engine.",
	...
	**/
	 ...
	 *
	 * @param test Boolean which is the result of the test you passed in
	 * @param message String which can be whatever you like to indicate what failed
	 * @param obj Object associated with message, normally 'this', may be null for statics
	**/
	public final static void concord_assert(boolean test, String message, Object obj)
	throws Error
	{
		if (test == false)
		{
			// Assert failed, treat the same as an ERROR level log message.
			log(ERROR, message, obj);

			// If we are in DEBUG mode, halt the calling thread.
			if (DEBUG)
			{
				throw new Error(EngineStrings.ASSERTION_FAILED + message);
			}
		}
    }
	...
	**/
	public final static void setLogLevel(int level)
	{
		if (level < 0)
		{
			myInstance.writeString(EngineStrings.INPUT_DEBUG_LOG_LEVEL_ILLEGAL,null);
		}
		else
		{
			logLevel = level;
			myInstance.writeString(EngineStrings.SETTING_DEBUG_LOG_LEVEL_TO + level, null);
		}
	}
		...
		if (DEBUG)
		{
			...
			if (messageLevel <= INFO && messageLevel <= logLevel)
			{
				if (messageLevel == ERROR)
					myInstance.writeString(EngineStrings.ERROR + message, obj);
				else if (messageLevel == WARNING)
					myInstance.writeString(EngineStrings.WARNING + message, obj);
				else if (messageLevel == INFO)
					myInstance.writeString(EngineStrings.INFO + message, obj);
				else
